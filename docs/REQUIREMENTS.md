...
- 标题"当前持仓"旁有 **↻ 刷新按钮**，点击后从 Yahoo Finance API 获取所有持仓的当前股价
- 包含列：**Ticker**, **股价**, **均价 * 股数**, **PnL**, **警报**, **自动止损**
- `股价` 显示当前股价（通过刷新按钮从 Yahoo Finance 获取）
- `PnL` 自动计算：`(当前股价 - 均价) * 股数`，盈利显示绿色 (`#059669`)，亏损显示红色 (`#dc2626`)，同时显示百分比变化
- `警报` 为下拉选择：✅ / ❌
- `自动止损` 为文本输入
- 数据存储在独立 CSV 文件中
